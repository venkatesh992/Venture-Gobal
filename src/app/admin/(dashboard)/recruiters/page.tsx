...
import { Search, Eye, Edit2, MoreVertical } from "lucide-react";
import Link from "next/link";
import { RecruiterSubmission } from "@prisma/client";

export const dynamic = "force-dynamic";

export default async function AdminRecruitersPage() {
  let recruiters: RecruiterSubmission[] = [];
  try {
    recruiters = await prisma.recruiterSubmission.findMany({
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          <h1 className="font-display text-3xl font-bold text-foreground tracking-tight">Recruiters</h1>
          <p className="mt-2 text-sm text-muted max-w-2xl">
            Manage recruiter applications and partnerships.
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-left text-[13px] text-muted">
            <thead className="bg-surface text-[10px] uppercase tracking-wider text-muted-light border-b border-border">
              <tr>
                ...
                <th className="px-5 py-4 font-semibold">Recruiter</th>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-[12px]">{recruiter.email}</span>
                            <span className="text-[12px] text-muted-light">{recruiter.phone}</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5">
                          {recruiter.status === 'pending' ? (
                            ...
                            <div className="flex items-center gap-1.5 rounded-full bg-blue-100 px-2 py-0.5 text-[11px] font-bold text-blue-600">
                              <span className="h-1.5 w-1.5 rounded-full bg-blue-500"></span> Contacted
                            </div>
                          ) : (
                            <div className="flex items-center gap-1.5 rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-bold text-emerald-600">
                              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500"></span> Reviewed
                            </div>
                          )}
                        </div>
                      </td>
                      <td className="px-5 py-4 font-mono text-[11px] text-muted-light">
                        REC-{recruiter.id.substring(recruiter.id.length - 4)}
                      </td>
                      <td className="px-5 py-4 text-[12px]">
                        {new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false }).format(new Date(recruiter.createdAt))}
                      </td>
                      <td className="px-5 py-4 text-right">
                        <div className="flex items-center justify-end gap-2 text-muted-light">
                          <Link href={`/admin/recruiters/${recruiter.id}`} className="p-1.5 hover:text-foreground hover:bg-slate-100 rounded-md transition-colors" title="View Details">
