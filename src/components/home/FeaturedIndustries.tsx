          ...
          >
            {industries.map((industry, index) => (
              ...
              >
                <Link 
                  href={industry.href}
                  className="group relative flex flex-col justify-between h-[360px] bg-white rounded-[1.5rem] p-6 border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.02)] hover:shadow-[0_20px_50px_rgb(0,0,0,0.08)] hover:border-blue-100 transition-all duration-500 overflow-hidden hover:-translate-y-1.5"
                >
                  {/* Decorative background gradient that appears on hover */}
                  <div className={`absolute top-0 right-0 w-56 h-56 bg-gradient-to-bl from-blue-50 to-transparent rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 -z-0 translate-x-8 -translate-y-8`} />

                  <div className="relative z-10">
                    <div className={`w-14 h-14 rounded-2xl ${industry.bgColor} ${industry.color} flex items-center justify-center mb-5 shadow-sm group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-500 ease-out`}>
                      <industry.icon size={26} strokeWidth={2.2} />
                    </div>
                    
                    <h3 className="font-display text-[22px] font-bold text-slate-900 mb-2 group-hover:text-[#0D34E9] transition-colors duration-300">
                      {industry.name}
                    </h3>
                    
                    <p className="text-slate-500 text-[14px] font-medium leading-relaxed line-clamp-3">
                      {industry.description}
                    </p>
                  </div>
                  
                  <div className="relative z-10 mt-6 flex items-center text-sm font-bold text-slate-400 group-hover:text-[#0D34E9] transition-colors duration-300">
                    <span>Explore Sector</span>
                    <svg className="w-4 h-4 ml-2 transform group-hover:translate-x-2 transition-transform duration-300 ease-out" fill="none" viewBox="0 0 24 24" stroke="currentColor">
